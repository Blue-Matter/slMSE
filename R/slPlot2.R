...
slplot.hist = function(obj, scol = c("red","green","blue","black","darkgrey","purple")){
  ...
  if(nStock(hist)>1)for(ss in 1:nStock(hist))matplot(xlab, Bage[[ss]],col=scol[ss],type='l',lty=1:nArea(hist),add=T)
  ...
  if(nStock(hist)>1)for(ss in 1:nStock(hist))matplot(xlab, Blist[[ss]],col=scol[ss],type='l',lty=1:nArea(hist),add=T)
  ...
  if(nStock(hist)>1)for(ss in 1:nStock(hist))matplot(FbyF[ss,,],col=scol[ss],type='l',lty=1:nFleet(obj),add=T)
  ...
  if(nStock(hist)>1)for(ss in 1:nStock(hist))matplot(xval, FbFy[ss,,],col=scol[ss],type='l',lty=1:nFleet(obj),add=T)
