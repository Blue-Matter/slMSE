...
proj_plot_mse = function(mat, lab, sims = 1:2, qsw = c(0.05,0.95), qsn = c(0.25,0.75), y0 = TRUE, ylim = NA, addn=T, addmed=T, addl = T,
...
MP_comp = function(MSEobj, varnam = "SBiomass", varlab = "Spawning Stock Biomass (t)", MPs = 1:2, denom = 1E3, stock= 1, fcols = c("#0000ff50","#ff000050","#00ff0050"),ns, annual = T, addn=T, addmed=T, addl = T){
  ...
  pt = MSEobj@OM@pYear*ns; nt = MSEobj@OM@nYear; lhy = max(Years(MSEobj,"Hist")); py = pt/ns; nsim = nSim(MSEobj); nmp = length(MSEobj@MPs)
  ...
  for(mp in MPs){
    mat = var[,,mp]
    proj_plot_mse(mat,lab, ylab = varlab,add=mp!=MPs[1],fcol=fcols[match(mp,MPs)],
...
slplot.mse = function(MSEobj, MPs = 1:2, annual = T, ns = 4, stock = 1,  MPcols = c("#0000ff50","#ff000050","#00ff0050"),
  ...
  # MSEobj =anMSE; MPs = 1:2; annual = T; ns = 4; stock = 1;  MPcols = c("#0000ff50","#ff000050","#00ff0050"); npy=20; MPnams = NA; leg=F; addn=F; addmed=T; addl = F
