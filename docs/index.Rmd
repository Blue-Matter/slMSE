...
To run the following scripts you will need to load up the following packages:


```{r packageload, eval=T,echo=F, results = 'hide', message=FALSE}
library(openMSE)
library(slMSE)
library(spict)
library(ggplot2)
...
```{r Simulate, eval = T, echo=T, message=FALSE}
...
```{r OptionalMSE, eval = T, echo = T, message=FALSE}
library(ggplot2)
...
```{r Spict_SimSam, eval = T, echo = T, message = FALSE}
...
```{r Spict_SimSam_q, eval = T, echo = T, message = FALSE}
# One simulation
...
```{r RCM_ASPM2, eval = T, echo = T, message = FALSE}
# Sim-Sam all simulations
...
```{r RCM_SCAL2, echo=T, eval = T, message = FALSE}
# Sim-Sam all simulations
...
```{r RCM_cond_1, eval=T, echo=T, message = FALSE}
...
Now that we have a conditioned seasonal operating model we can test management procedures. 

OpenMSE comes with a few examples:


```{r availmp, eval=T, echo=T}
avail('mp')
```

These are pretty basic but the OpenMSE selection will grow with future releases (please make requests on the MSEtool GitHub Issues page!)

The slMSE packages comes with a Generic Index Ratio (GIR) management procedure of the type used to provide advice in stocks such as Atlantic Bluefin Tuna and North Atlantic Swordfish. 

Essentially the MP aims to adjust TACs based on an index of abundance. The MP is calibrated such that TAC advice is relative to catches in the last historical year (e.g. 120 tons in 2026) and future index levels in the last historical year (e.g. 36.1821 in 2026). The MP is names 'generic' because we are not supplying an observed historical index of abundance but rather using the internally generated relative biomass index. It is trivial to add real data and simulate the statistical properties of a real index, we just don't have these!

To find out more about GIR you can use the in-line help:


```{r GIR_Help, echo=T, eval = T}
?GIR
```

The GIR has a number of features. It can smooth indices with a polynomial to remove noise from signal, it can limite upward and downward changes in TAC and can use control points that impact how advice will be generated.

We can actually plot a GIR configuration by modifying the function (to plot) and using an example dataset that was loaded with slMSE:

```{r GIR_HCR, echo=T, eval = T, message = FALSE}

plotGIR = GIR   # copy function
formals(plotGIR)$plot = T

par(mfrow=c(1,2))
plotGIR(Example_Data)

```

Lets run the default GIR MP along with a version 'myGIR' that we have modified:

```{r GIRMSE, eval=T, echo = T, message = FALSE}
...
## Specifying Alternative Projection dynamics
